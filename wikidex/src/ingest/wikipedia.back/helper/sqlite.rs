...
pub(crate) async fn write_completion_timestamp(
    ...
    .await
    .map_err(SqlX)?;
    // let rows = sqlx::query_with(&query).execute(connection).await.map_err(SqlX)?;
...
pub(crate) async fn count_elements(pool: &SqlitePool) -> Result<Option<i64>, IngestError> {
    let mut connection = pool.acquire().await.map_err(SqlX)?;
...
pub(crate) async fn get_sqlite_pool<P: AsRef<Path>>(path: &P) -> Result<SqlitePool, IngestError> {
    ...
    if !sqlx::Sqlite::database_exists(&path).await.map_err(SqlX)? {
        sqlx::Sqlite::create_database(&path).await.map_err(SqlX)?;
    }

    SqlitePool::connect(&path).await.map_err(SqlX)
}
